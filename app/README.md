...
**Pendiente:** las entidades con casos polimórficos propios (`IPilotGearData`, `INpcFeatureData`, `IFrameData`, `IMechSystemData`, `IWeaponModData`, `INpcClassData`, `INpcTemplateData`, `IEidolonLayerData`) — mismo mecanismo `CatalogFieldSpec` ya usado en arma para uniones cerradas, pero más trabajo por entidad. Reserve/CoreBonus/Talent (reutilizan el "paquete" de actions/bonuses/synergies/deployables) también quedan pendientes — antes de abordarlas conviene extraer ese paquete de `weapon_form_schema.dart` a un módulo común, ya que hoy vive atrapado ahí.

Tests: mapper JSON (unitarios, incluidas las 8 entidades nuevas), aceptación end-to-end del caso de uso (genera un `.lcp` real, lo abre como zip, verifica contenido) incluyendo un test de exportación multi-contenido (varias entidades en el mismo `.lcp`), ensamblador del formulario (unitarios, arma + las 8 simples), widget tests de `MultiEnumFieldSpec`/`GroupFieldSpec`/catálogo-en-lista/`CrearEntidadScreen` genérico, y smoke test de la app completa (menú → formulario de arma) — cumple la condición de tests bloqueante de ADR-002. `flutter test`: 46/46 pasan. No se pudo ejecutar `flutter run` en este entorno remoto (sin display gráfico) — verificado con tests de widget en su lugar.
