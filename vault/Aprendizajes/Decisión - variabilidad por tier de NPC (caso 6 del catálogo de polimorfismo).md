...
**Consecuencia práctica:** el caso 6 no necesita ningún `FieldSpec` nuevo — se resuelve combinando `ShapeChoiceFieldSpec` (generalizado a N ramas) y `GroupFieldSpec` (ya existente, reutilizado sin cambios), con la diferencia de tipo de dato resuelta en cada función ensambladora. Pendiente de implementar en `field_spec.dart`/`generic_form_view.dart` (esta nota documenta la decisión de diseño, no su implementación todavía).
